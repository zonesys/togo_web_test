    ...
    const RowComponent = ({ record, num, style }) => (
        <div style={style} className={"list-group-item-custom"}>
            <div className="d-flex justify-content-between">
                <div style={{ width: "40%" }}>
                    <div style={{ fontWeight: "bold", color: "#03A65A" }}>{record.customer_mobile}</div>
                    <div>{record.time_stamp.split(" ")[0] + " "}<Badge bg="primary">{record.time_stamp.split(" ")[1]}</Badge></div>
                </div>
                <div style={{ width: "70%" }}>{"Charge Balance " + record.amount + " NIS to " + record.customer_type + " " + record.customer_name}</div>
            </div>
        </div>
    );
